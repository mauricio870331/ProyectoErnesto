...
//    eliminar el token
    ...
    $sql_ = "select count(documento) total from empleados";
    $rs_c = $con->findAll2($sql_);
...
//    echo $query;
    $row = $con->exec($query);
    $con->desconectar();
    echo json_encode("Filas Agregadas: " . $row["row_count"]);
}
...
if ($method == "PUT") {
    ...
    $row = $con->exec($query);
    $con->desconectar();
    echo json_encode("Filas Actualizadas: " . $row["row_count"]);
}
